...
export async function encryptMasterKey(
	...
	const iv = crypto.getRandomValues(new Uint8Array(IV_SIZE)) as Uint8Array<ArrayBuffer>;
	const rawMasterKey = await crypto.subtle.exportKey('raw', masterKey);

	const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, passwordKey, rawMasterKey);
...
export async function encryptText(
	...
	const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, masterKey, encoded);
